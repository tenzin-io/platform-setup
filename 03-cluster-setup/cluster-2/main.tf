terraform {
  ...
  backend "s3" {
    bucket         = "tenzin-io"
    key            = "terraform/cluster.vm.vhost-2.state"
...
module "prometheus" {
  ...
  prometheus_fqdn     = "prometheus.vm.vhost-2"
...
module "jupyterhub" {
  depends_on                   = [module.local_path_provisioner, module.nvidia_gpu_operator]
